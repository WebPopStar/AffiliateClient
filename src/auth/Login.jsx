...
const Login = () => {
	...
	return (
		<>
			{!show ? (<><Heading label={"Log Account"} />
				<AppServerErr>
				...
				<form className="mt-6 font-sans" onSubmit={formik.handleSubmit}>
					...
					/>
					<AppServerErr>
						{op.appErr === "Affiliate or Password not correct." &&
							op.appErr}
					</AppServerErr>

					<SignButton label={"Login"} />
					<Hint label={'Register'} question={'Do you wnat to register?'} handle={() => navigate('/register')} />
					<Hint label={'Forgot Password?'} className='text-red-600  ' handle={() => setShow(true)} />
				</form>
